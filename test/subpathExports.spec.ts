...
describe('subpath exports', () => {
  ...
  it('src/config exposes snapshot + validator helpers', async () => {
    const mod = await import('src/config/index.js');
    expect(typeof mod.loadScallopConfigSnapshot).toBe('function');
    expect(typeof mod.createScallopConfigSnapshot).toBe('function');
    expect(typeof mod.validateConfigSnapshot).toBe('function');
    expect(typeof mod.assertConfigSnapshot).toBe('function');
    expect(Array.isArray(mod.REQUIRED_CORE_PATHS)).toBe(true);
    expect(Array.isArray(mod.REQUIRED_WHITELIST_KEYS)).toBe(true);
  });

  it('src/context exposes createScallopContext', async () => {
    const mod = await import('src/context/index.js');
    expect(typeof mod.createScallopContext).toBe('function');
  });
  ...
  it('src/mappers exposes parseMoveTypeName + event mappers', async () => {
    const mod = await import('src/mappers/index.js');
    expect(typeof mod.parseMoveTypeName).toBe('function');
    expect(typeof mod.mapTypeNameField).toBe('function');
  });
