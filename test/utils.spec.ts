...
beforeAll(async () => {
  ...
  SUPPORT_POOLS = [...scallopUtils.address.getWhitelist('lending')];
  SUPPORT_SCOIN = [...scallopUtils.address.getWhitelist('scoin')];
});
...
describe('Test Scallop Utils', () => {
  ...
  it('Should parse to coin type from coin name', () => {
    ...
    const suiAssertCoinType = `${scallopUtils.constants.get('core.coins.sui.id')}::sui::SUI`;
    const usdcAssertCoinType = `${scallopUtils.constants.get(
      'core.coins.wusdc.id'
    )}::coin::COIN`;
    const usdtAssertCoinType = `${scallopUtils.constants.get(
      'core.coins.wusdt.id'
    )}::coin::COIN`;
    const afsuiAssertCoinType = `${scallopUtils.constants.get(
      'core.coins.afsui.id'
    )}::afsui::AFSUI`;
    const vsuiAssertCoinType = `${scallopUtils.constants.get(
      'core.coins.vsui.id'
    )}::cert::CERT`;
    if (ENABLE_LOG) {
  ...
  it('Should parse to market coin type from coin name', () => {
    ...
    const suiAssertMarketCoinType = `${scallopUtils.constants.protocolObjectId}::reserve::MarketCoin<${scallopUtils.constants.get(
      'core.coins.sui.id'
    )}::sui::SUI>`;
    const usdcAssertMarketCoinType = `${scallopUtils.constants.protocolObjectId}::reserve::MarketCoin<${scallopUtils.constants.get(
      'core.coins.wusdc.id'
    )}::coin::COIN>`;
    const usdtAssertMarketCoinType = `${scallopUtils.constants.protocolObjectId}::reserve::MarketCoin<${scallopUtils.constants.get(
      'core.coins.wusdt.id'
    )}::coin::COIN>`;
    const vsuiAssertMarketCoinType = `${scallopUtils.constants.protocolObjectId}::reserve::MarketCoin<${scallopUtils.constants.get(
      'core.coins.vsui.id'
    )}::cert::CERT>`;
    if (ENABLE_LOG) {
  ...
  it('Should parse to scoin type from coin name', () => {
    SUPPORT_SCOIN.forEach((t) => {
      ...
      expect(scoinType).toBe(
        scallopUtils.constants.get(`scoin.coins.${t}.coinType`)
      );
    });
  });
